...
import { faUserTag, faImages } from "@fortawesome/free-solid-svg-icons";
...
const TagNotificationCard = ({
  ...
  const respondMut = useMutation({
    ...
    onMutate: async (action: "accepted" | "declined") => {
      ...
      const mutation = respondToTagMutation(notification.id);
      if (mutation.onMutate) {
        return mutation.onMutate(action);
      }
      return {};
    },
    onSettled: () => {
  ...
  const isPending =
    notification.type === "TAG" && notification.status === "pending";
  ...
  return (
    <div className="py-5 px-4 border-b-2 border-gray-100 flex flex-col transition-colors hover:bg-gray-50/50">
      ...
      <div className="flex gap-3 items-start">
        ...
        >
          {notification.actor.profilePicUrl ? (
            <img
              ...
              className="w-10 h-10 rounded-full object-cover shadow-sm hover:opacity-90 transition-opacity"
            />
          ) : (
            <div className="w-10 h-10 rounded-full bg-gray-50 border border-gray-200 flex items-center justify-center hover:opacity-90 transition-opacity">
              <FontAwesomeIcon
                icon={faUserTag}
        ...
        <div className="flex-1 min-w-0 self-center">
          ...
          <div className="flex items-center gap-2 mt-1">
            ...
            {notification.type === "TAG" && notification.status && (
              <>
      ...
      {/* Big Vault Cover */}
      <div onClick={handleVaultClick} className="cursor-pointer mt-3 w-full">
