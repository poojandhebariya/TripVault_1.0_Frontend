...
const CreateVault = () => {
  ...
  const uploadAllAttachments = async (): Promise<boolean> => {
    ...
    for (const att of pending) {
      ...
      try {
        ...
        setAttachments((p) =>
          p.map((a) =>
            a.id === att.id
              ? { ...a, uploadedUrl: url, isUploading: false }
              : a,
          ),
        );
      } catch {
        success = false;
        setAttachments((p) =>
          p.map((a) => (a.id === att.id ? { ...a, isUploading: false } : a)),
        );
        showSnackbar({
  ...
  const validateAndSubmit = async (
    ...
    if (hasPending) {
      const allOk = await uploadAllAttachments();
      if (!allOk) return; // upload errors already shown
    }

    // Re-read state to get latest uploadedUrls after the upload
    const uploadedAttachments: VaultAttachment[] = attachments.map((a) => ({
      url: a.uploadedUrl ?? "",
      type: a.type,
    }));
